...
public class NormalizedCutVertexProgram implements VertexProgram<Pair<Serializable, Double>> {

    private static final Logger LOGGER = LoggerFactory.getLogger(NormalizedCutVertexProgram.class);

    @SuppressWarnings("WeakerAccess")
    public static final String NORMALIZED_CUT = "ru.sfedu.test.NormalizedCutVertexProgram.normalizedCut";

    public static final String CUT_EDGES_WEIGHT_PROPERTY = "ru.sfedu.test.NormalizedCutVertexProgram.cutEdgesWeightProperty";
    public static final String VERTEX_DEGREE_PROPERTY = "ru.sfedu.test.NormalizedCutVertexProgram.vertexDegreeProperty";

    private static final String EDGE_TRAVERSAL = "ru.sfedu.test.NormalizedCutVertexProgram.edgeTraversal";
    private static final String WEIGHT_TRAVERSAL = "ru.sfedu.test.NormalizedCutVertexProgram.weightTraversal";
    private static final String CLUSTER_TRAVERSAL = "ru.sfedu.test.NormalizedCutVertexProgram.clusterTraversal";
    ...
    private final Set<VertexComputeKey> VERTEX_COMPUTE_KEYS = new HashSet<>(Arrays.asList(
            VertexComputeKey.of(CUT_EDGES_WEIGHT_PROPERTY, false),
            VertexComputeKey.of(VERTEX_DEGREE_PROPERTY, false)));

    private final Set<MemoryComputeKey> memoryComputeKeys = new HashSet<>();

    private NormalizedCutVertexProgram() {
    ...
    public VertexProgram<Pair<Serializable, Double>> clone() {
        try {
            final NormalizedCutVertexProgram clone = (NormalizedCutVertexProgram) super.clone();
    ...
    public Set<MapReduce> getMapReducers() {
        ...
        mapReducers.add(NormalizedCutMapReduce.build().clusterProperty(clusterProperty).create());
        return mapReducers;
    }
    ...
    public void execute(final Vertex vertex, final Messenger<Pair<Serializable, Double>> messenger, final Memory memory) {
        ...
        if (memory.isInitialIteration()) {
            ...
            vertex.property(VertexProperty.Cardinality.single, VERTEX_DEGREE_PROPERTY, vertexIncidentWeights);
            ...
            vertex.property(VertexProperty.Cardinality.single, CUT_EDGES_WEIGHT_PROPERTY, cutEdgesWeight);
        }
    }
    ...
    public static final class Builder extends AbstractVertexProgramBuilder<Builder> {
        ...
        private Builder() {
            super(NormalizedCutVertexProgram.class);
        }
